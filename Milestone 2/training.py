...
# data, labels, test_data, test_labels = load_data(data_directory)
# data_normalized = normalize_data(data)
# # Assume data_normalized and labels are already defined
# mean = np.mean(data_normalized, axis=0)
# std = np.std(data_normalized, axis=0)

# # Save mean and std to a file for later use in testing
...
train_dataset = TensorDataset(torch.tensor(X_train).to(torch.float32), torch.tensor(y_train).to(torch.long))
val_dataset = TensorDataset(torch.tensor(X_val).to(torch.float32), torch.tensor(y_val).to(torch.long))
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)
val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            # Calculate collision accuracy
            collision_mask = labels == 1
            collision_correct += (outputs.argmax(1)[collision_mask] == labels[collision_mask]).sum().item()
            collision_total += collision_mask.sum().item()
    val_acc = val_correct / val_total
    collision_acc = collision_correct / collision_total if collision_total > 0 else 0
    print(f'Epoch {epoch+1}/{epochs}, Train Loss: {loss.item()}, Train Acc: {train_acc:.4f}, Val Acc: {val_acc:.4f}, Collision Acc: {collision_acc:.4f}')
